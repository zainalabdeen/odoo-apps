    ...
    def _create_picking(self):
        ...
        if not self.multi_deliver:
            ...
            for order in self.filtered(lambda po: po.state in ('purchase', 'done')):
                ...
                for picking in order.deliver_line_ids.mapped('picking_type_id'):
                    if any([ptype in ['product', 'consu'] for ptype in order.deliver_line_ids.filtered(lambda s: s.picking_type_id.id== picking.id).mapped('product_id.type')]):
                        res = order._prepare_to_multi_picking(picking)
                        created_picking = StockPicking.with_user(SUPERUSER_ID).create(res)
                        for line in order.deliver_line_ids.filtered(lambda s: s.picking_type_id.id== picking.id):
                            ...
                            created_picking.message_post_with_view('mail.message_origin_link',
                                                    values={'self': created_picking, 'origin': order},
